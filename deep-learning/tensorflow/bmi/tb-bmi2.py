...
# define data flow
# define placeholder
x = tf.placeholder(tf.float32, [None, 2], name="x") # height and weight
y_ = tf.placeholder(tf.float32, [None, 3], name="y_") # label

# with name scope for tensorboard
with tf.name_scope('interface') as scope:
  # define variables
  W = tf.Variable(tf.zeros([2, 3]), name="W") # weight(not equal to the above)
